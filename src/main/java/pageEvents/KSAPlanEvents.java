...
public class KSAPlanEvents extends BaseTest {
	...
	public void verify_Liteprice() {
		...
		if (flag == 1) {
			logger.fail("Lite Price is not numeric in KSA Subsription");
		}
	}
	...
	public void verify_Classicprice() {
		...
		if (flag == 1) {
			logger.fail("classicprice is not numeric in KSA Subsription");
		}
	}
	...
	public void verify_Premiumprice() {
		...
		if (flag == 1) {
			logger.fail("premiumprice is not numeric in KSA Subsription");
		}
	}
	...
	public void verify_litecurrency() {
		...
		if (iend != -1) {
			...
			if (subString.equals("SAR")) {
				Assert.assertEquals(subString, "SAR");
				logger.pass("Lite curreny is correct in Lite Subscription");
			} else {
				logger.fail("KSA curreny is wrong in Lite Subscription");
			}
		}
	...
	public void verify_classiccurrency() {
		...
		if (iend != -1) {
			...
			if (subString.equals("SAR")) {
				...
				logger.fail("KSA curreny is wrong in Classic Subscription");
			}
		}
	...
	public void verify_premiumcurrency() {
		...
		if (iend != -1) {
			...
			if (subString.equals("SAR")) {
				...
				logger.fail("KSA curreny is wrong in Premium Subscription");
			}
		}
